...
class FaissClient:
    ...
    def _load_index(self) -> faiss.IndexIDMap2:
        ...
        if not self._index_filepath.exists():
            ...
            logger.info(
                "FAISS index not found, creating new index at %s",
        ...
        try:
            ...
            if index.d != self._dimensions:
                raise ValueError(
                    f"Dimension mismatch: index has {index.d}, but expected {self._dimensions}",
                )
            logger.info(
                "Loaded FAISS index from %s (ntotal=%d, dim=%d)",
                self._index_filepath,
                index.ntotal,
                index.d,
            )
            ...
            logger.warning(
                "Failed to read FAISS index at %s, creating new empty index",
                self._index_filepath,
            )
            return self._create_index_id_map_index()
    ...
    def write_index(self) -> None:
        ...
        """
        logger.debug(
            "Writing FAISS index to %s (ntotal=%d)",
            self._index_filepath,
            self._index.ntotal,
        )
    ...
    def write(self, embedding: Embedding, ids: list[int]) -> None:
        ...
        logger.debug(
            "Adding %d embeddings to FAISS index (ntotal before=%d)",
    ...
    def _normalize_embedding(self, embedding: Embedding) -> Embedding:
        ...
        if embedding.dtype != np.float32:
            raise TypeError(
                f"dtype must be np.float32 type for FAISS. Got {embedding.dtype}",
            )
        if embedding.ndim != 2 or embedding.shape[1] != self._dimensions:
            raise ValueError(
                f"Expected shape (*, {self._dimensions}), got {embedding.shape}",
            )
    ...
    def query(self, embedding: Embedding, k: int) -> tuple[Distances, Ids]:
        ...
        normalized_embedding = self._normalize_embedding(embedding)
        distances, ids = self._index.search(normalized_embedding, k)
